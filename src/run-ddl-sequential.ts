...
// ----------------------------- Helpers -----------------------------
function listJobs() {
  const files = fs.readdirSync(MIG_DIR).filter(f => f.endsWith(".sql")).sort(); // apply in order
  return files.map(f => {
...
// ----------------------------- Main -----------------------------
async function main() {
  console.log(`Running SEQUENTIAL DDL from: ${MIG_DIR}  (Atlas mark: ${DO_ATLAS_MARK ? "on" : "off"})`);
  ...
  console.log(`sequential-total: ${totalMs}ms`);
  writeLogs("sequential", rows, totalMs);
